...
var utils = {
    ...
    checkDate: function (date) {
        ...
        if (yy < 1900 || mm < 1 || mm > 12 || dd < 1) {
            return false;
        }
        
        if ([1, 3, 5, 7, 8, 10, 12].indexOf(mm) !== -1 && dd > 31) {
            return false;
        } else if (mm === 2 && (this.isLeapYear(yy) && dd > 29) || (!this.isLeapYear(yy) && dd > 28)) {
            return false;
        } else if (dd > 30) {
            return false;
        }

        if (new Date(yy, mm - 1, dd) > nowDate) return false;
